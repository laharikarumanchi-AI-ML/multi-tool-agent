...
│   ├── test_end_to_end.py
│   └── test_eval.py
├── app.py                        # Streamlit, deployed to HF Spaces. Lives at
│                                 # repo root (not under demo/) per the HF
│                                 # Spaces convention — Spaces looks for
│                                 # app.py at root by default. Revised
│                                 # 2026-06-08; see §3.10 revision note.
└── docs/superpowers/
    ├── specs/                    # This document
...
| Multi-search synthesis | 4 | "City where 2024 Nobel literature winner was born + its population" | search × N |
| Datetime reasoning | 5 | "How many years between Apple's founding and the iPhone launch?" | search, datetime |
| Unit conversion + facts | 7 | "Is the speed of sound at sea level higher than a Boeing 747's top speed in m/s?" | search, unit_convert, calculator |
| Multi-tool freestyle | 3 | "How many minutes would it take to drive NYC→LA at average highway speed?" | search, unit_convert, calculator |
...
Revised 2026-06-08 after PR #7 authoring: counts shifted from the original
spec (6/6/5/4/4) to match what landed in `multitool/eval/test_set.jsonl`.
Two categories drifted while hand-curating queries — unit-conversion turned
out to have more interesting multi-tool variants than originally budgeted
(7 vs 4), which absorbed slots from multi-search-synthesis (4 vs 6) and
multi-tool-freestyle (3 vs 4). Total still sums to 25; "balanced" is now
closer to "weighted toward the categories where the loop has the most
to prove."
...
`app.py` (at repo root) — Streamlit app with:
...
- Example chips: 5-7 curated queries (one click populates the input)
...
Revised 2026-06-08:
- Location: `demo/app.py` → `app.py` at repo root. HF Spaces looks for
  `app.py` at the root by default; nesting it under `demo/` would require
  either duplicating the file or configuring `app_file:` in the YAML
  frontmatter, both of which add friction for no gain. Earlier drafts of
  the spec had this as an internal inconsistency (directory tree said
  `demo/app.py`; the HF Spaces YAML frontmatter said `app_file: app.py`).
- Chip count: "5-6" → "5-7". The Streamlit demo currently ships 7 chips,
  picked for category variety across the 5 eval categories; the original
  5-6 budget proved too tight to showcase the spread.

### 3.10.1 CLI

`python -m multitool ask "<question>"` — argparse-driven subcommand interface
(no Click) so future commands (`multitool eval`, `multitool demo`) can be
added cleanly. The `ask` subcommand accepts:

| Flag | Default | Purpose |
|---|---|---|
| `--provider {groq,gemini}` | `groq` | LLM provider. Requires `GROQ_API_KEY` or `GEMINI_API_KEY` accordingly. |
| `--model MODEL` | provider default | Override the default model for the chosen provider (e.g. `--model llama-3.1-8b-instant`). Useful for cheap-fast smoke tests vs. the eval-grade model. |
| `--trace PATH` | `traces/` | Directory to write trace JSON files into. ALWAYS treated as a directory; created if missing. See "trace directory semantics" below. |
| `-v`, `--verbose` | off | Print each tool call (name, args, truncated result) to stderr after the run, plus a `[done in N steps; trace: ...]` summary. Also re-raises orchestrator exceptions with the full traceback (without `-v`, exceptions become a one-line friendly stderr message). |

**Trace directory semantics.** `--trace PATH` is always treated as a
directory, even when `PATH` has no trailing slash or extension. The
orchestrator's `Trace` class always derives its own `<run_id>.json`
filename inside the directory; `--trace` lets the user redirect *where*
traces land, not rename them. This avoids the foot-gun where
`os.path.dirname("mytrace")` returns `""` and traces silently drop into
the current working directory. The directory is created (`mkdir -p`) if
missing — both for user-supplied paths and the default `traces/`. Codified
by PR #8's fix-up commit (`fix(cli): treat --trace PATH as directory,
always`).

**Exit codes:**
- `0` — success, final answer printed to stdout.
- `1` — missing required env var, orchestrator raised, or
  `error="max_steps_reached"` (orchestrator does not surface partial
  state; the trace file is the recovery artifact).
- `2` — argparse usage error (missing positional, unknown subcommand, bad
  `--provider` choice).

Revised 2026-06-08: original spec only mentioned `--provider {groq,gemini}`;
`--model`, `--trace`, and `-v/--verbose` were added during PR #8
implementation as real UX wins (cheap-model smoke tests, redirectable
trace dirs, opt-in verbose debugging) and should have been in the spec
from the start.
...
Reality-vs-spec audit after the 10 planned PRs landed. Each entry is a
drift that flowed spec ← reality because reality was the right behavior
and the spec was wrong, behind, or incomplete. Code was not touched in
this audit.

| Date | Section | Drift | Resolution |
|---|---|---|---|
| 2026-06-08 | §3.1 (repo layout) | Tree showed `demo/app.py`; reality has `app.py` at repo root (HF Spaces convention). | Tree updated to `app.py` at root; cross-references to §3.10. |
| 2026-06-08 | §3.8 (eval test set) | Category counts in the table (6/6/5/4/4) didn't match `multitool/eval/test_set.jsonl` (6/4/5/7/3). | Counts updated to match the JSONL; rationale paragraph added. |
| 2026-06-08 | §3.10 (demo) | Spec said `demo/app.py`; YAML frontmatter said `app_file: app.py` (internal inconsistency). | `app.py` at repo root is now the canonical location. |
