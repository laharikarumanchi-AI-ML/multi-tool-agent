...
class TestChatWithToolsContractGuards:
    ...
    def test_groq_tool_use_failed_400_retries_then_succeeds(self, mocker):
        """Regression: Groq returns HTTP 400 with code='tool_use_failed'
        when the MODEL emits malformed function-call syntax (raw Python
        like `pint.Quantity(...)`, multiple `<function=...>` blocks, etc.).
        This is a transient model-output issue — the next sample often
        parses cleanly. We retry it like the other retryable statuses
        instead of crashing the whole agent run.

        This was the actual root cause of the 9/25 eval HTTP 400 failures.
        """
        from multitool.llm_client import GroqClient

        # First attempt: 400 tool_use_failed. Second attempt: 200 OK.
        fail_resp = mocker.MagicMock()
        fail_resp.status_code = 400
        fail_resp.ok = False
        fail_resp.text = '{"error":{"code":"tool_use_failed"}}'
        fail_resp.headers = {}
        fail_resp.json.return_value = {
            "error": {
                "message": "Failed to call a function.",
                "code": "tool_use_failed",
                "failed_generation": "<function=foo>{bad python}</function>",
            }
        }

        ok_resp = mocker.MagicMock()
        ...
        post_mock = mocker.patch(
            "multitool.llm_client.requests.post",
            side_effect=[fail_resp, ok_resp],
        )
        # Avoid the actual time.sleep slowing the test
        mocker.patch("multitool.llm_client.time.sleep")

        client = GroqClient(api_key="dummy")
        result = client.chat_with_tools(messages=[], tools=[])
        assert result.content == "Hi"
        # Two attempts: the 400 was retried.
        assert post_mock.call_count == 2

    def test_groq_tool_use_failed_400_eventually_raises_after_max_attempts(self, mocker):
        """If every attempt returns tool_use_failed, we eventually raise
        (don't retry forever). Mirrors the cap on retryable statuses."""
        from multitool.llm_client import GroqClient
        ...
        fail_resp.text = '{"error":{"code":"tool_use_failed"}}'
        fail_resp.headers = {}
        fail_resp.json.return_value = {"error": {"code": "tool_use_failed"}}
        fail_resp.raise_for_status.side_effect = _requests.HTTPError(
            "400 Client Error", response=fail_resp
        )

        post_mock = mocker.patch(
            "multitool.llm_client.requests.post", return_value=fail_resp
        )
        mocker.patch("multitool.llm_client.time.sleep")
        ...
        assert post_mock.call_count == GroqClient.MAX_ATTEMPTS

    def test_groq_400_non_tool_use_failed_raises_immediately(self, mocker):
        """Other 400s (bad auth, invalid schema, malformed JSON) should still
        raise immediately — they won't fix themselves on retry."""
        from multitool.llm_client import GroqClient
        import requests as _requests
        ...
        bad_resp.ok = False
        bad_resp.text = '{"error":{"code":"invalid_api_key"}}'
        bad_resp.json.return_value = {"error": {"code": "invalid_api_key"}}
        bad_resp.raise_for_status.side_effect = _requests.HTTPError(
            "400", response=bad_resp
        )
        post_mock = mocker.patch(
            "multitool.llm_client.requests.post", return_value=bad_resp
        )
        ...
        # Only one attempt — no retry on non-tool_use_failed 400.
        assert post_mock.call_count == 1
